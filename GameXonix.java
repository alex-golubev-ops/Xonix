...
class GameXonix extends JFrame {
    ...
    final String TITLE_OF_PROGRAM = "Ксоникс";
    final int POINT_SIZE = 10;
    final int FIELD_WIDTH = 650 / POINT_SIZE;
    final int FIELD_HEIGHT = 450 / POINT_SIZE;
    ...
    void go() {
        while (!gameover.isGameOver()) {
            ...
            cube.move();
            canvas.repaint();
            board.setText(String.format(FORMAT_STRING, field.getCountScore(), "Жизни:", xonix.getCountLives(), "Площадь:", field.getCurrentPercent()));
    ...
    class Xonix {
        private int x, y, direction, countLives = 10;
        ...
        void paint(Graphics g) {
            g.setColor((field.getColor(x, y) == COLOR_LAND) ? new Color(0x7B0562) : Color.white);
            g.fillRect(x*POINT_SIZE, y*POINT_SIZE, POINT_SIZE, POINT_SIZE);
            g.setColor((field.getColor(x, y) == COLOR_LAND) ? Color.white : new Color(0xF5AC1B));
    ...
    class Cube {
        ...
        void paint(Graphics g) {
            g.setColor(new Color(0xFF470202, true));
            g.fillRect(x*POINT_SIZE, y*POINT_SIZE, POINT_SIZE, POINT_SIZE);
    ...
    class GameOver {
        private final String GAME_OVER_MSG = "Ну ты и ЛУЗЕР";
